...
import { Edit, ArrowLeft, Plus, X, Search, Clock, UtensilsCrossed, Sparkles, Link2, Check } from "lucide-react";
...
function SchemaDetailContent({ schemaId }: { schemaId: string }) {
  ...
  const [hadSavedWeekMenu, setHadSavedWeekMenu] = useState(false);
  ...
  const copyClientLink = async () => {
    const url = clientLink || `${window.location.origin}/my-plan/${schemaId}`;
    try {
  ...
  useEffect(() => {
    setClientLink(`${window.location.origin}/my-plan/${schemaId}`);
  }, [schemaId]);
      ...
      <div className="page-section" style={{ marginTop: 0 }}>
        ...
        >
          ...
          <div style={{ display: "flex", gap: "0.75rem", alignItems: "center", flexWrap: "wrap" }}>
            ...
            >
              {clientLink || `/my-plan/${schemaId}`}
            </code>
            <button type="button" className="btn btn--secondary" onClick={copyClientLink}>
              {linkCopied ? <Check size={16} /> : <Link2 size={16} />}
          ...
          {hadSavedWeekMenu ? (
            <span style={{ fontSize: "0.8rem", color: "#64748b" }}>
              Weekmenu is opgeslagen — klanten zien dit plan via de link.
            </span>
          ) : (
            <span style={{ fontSize: "0.8rem", color: "#64748b" }}>
              Weekmenu wordt opgeslagen zodat de klantlink direct werkt…
            </span>
          )}
        </div>
      </div>
